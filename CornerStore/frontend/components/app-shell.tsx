      ...
      <div className="min-h-screen text-text">
        <header className="sticky top-0 z-50 border-b border-border glass-strong">
          <div className="container-premium flex h-14 items-center justify-between">
            <Link href="/admin" className="section-title text-lg font-bold">
              <span suppressHydrationWarning>{t("cornerStoreAdmin", language)}</span>
            </Link>
            <div className="flex items-center gap-2">
              <button type="button" className="rounded-lg border border-border px-2 py-1 text-xs" onClick={toggleTheme}>
                <span suppressHydrationWarning>{theme === "dark" ? t("lightMode", language) : t("darkMode", language)}</span>
              </button>
              <Link href="/" className="rounded-lg border border-border px-3 py-1.5 text-sm text-text-muted hover:bg-surface-2">
                <span suppressHydrationWarning>{t("storefront", language)}</span>
              </Link>
            </div>
          </div>
        </header>
      ...
      <header className="sticky top-0 z-50 border-b border-border glass-strong">
        <div className="container-premium flex h-[var(--nav-height)] items-center gap-4">
          ...
          <div className="ml-auto flex items-center gap-1 sm:gap-2">
            ...
            <NavIcon href="/compare" label={t("compare", language)} active={pathname === "/compare"} badge={compareIds.length}>
            ...
            {isSignedIn ? (
              <NavIcon href="/wishlist" label={t("wishlist", language)} active={pathname === "/wishlist"} badge={wishlistCount}>
            ...
            <NavIcon href="/cart" label={t("cart", language)} active={pathname === "/cart"} badge={cartCount}>
            ...
            <NavIcon href="/notifications" label={t("notifications", language)} active={pathname === "/notifications"} badge={unreadNotifications}>
            ...
            {isSignedIn ? (
              <>
                ...
                {isAdmin ? (
                  ...
                  >
                    <span suppressHydrationWarning>{t("admin", language)}</span>
                  </Link>
                ) : null}
                <button type="button" className="hidden rounded-xl px-3 py-2 text-sm text-text-muted hover:bg-surface-2 lg:inline-flex" onClick={signOut}>
                  <span suppressHydrationWarning>{t("signout", language)}</span>
                </button>
              </>
            ) : (
              <>
